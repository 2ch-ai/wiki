...
!!! danger "Важно"
	Модели детекторов используют формат .pt (pickletensor), который менее безопасен, чем .safetensors. Устанавливайте pickletensors только из источников, которым доверяете. Как и с кастомными нодами, требуется собственная должная осмотрительность. Если не хотите запускать pickletensors, маску для лица и рук можно нарисовать вручную — это несложно и работает хорошо. См. раздел [Инпейнтинг](#инпейнтинг) для более подробной информации.
