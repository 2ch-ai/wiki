...
ControlNet - это нейросетевая модель для управления [моделями Stable Diffusion](https://stable-diffusion-art.com/how-stable-diffusion-work/). Вы можете использовать ControlNet с любыми моделями Stable Diffusion.

Самый базовый способ использования моделей Stable Diffusion - это text-to-image (txt2img). Он использует текстовые промпты в качестве [условия](https://stable-diffusion-art.com/how-stable-diffusion-work/#Conditioning) для управления генерацией изображений, чтобы вы могли создавать изображения, соответствующие текстовому описанию.
...
Если вы хорошо знакомы с командной строкой, вы можете использовать этот вариант для обновления ControlNet, который дает вам уверенность в том, что Web-UI не делает что-то еще.
...
Для следования этому разделу у вас должно быть установлено расширение ControlNet. Вы можете убедиться в этом, увидев раздел ControlNet ниже.

![](https://stable-diffusion-art.com/wp-content/uploads/2023/02/image-115.png)

Нажмите на каретку справа, чтобы развернуть панель ControlNet. Она покажет полный раздел элементов управления и холст для загрузки изображений.

![](https://stable-diffusion-art.com/wp-content/uploads/2023/05/image-91.png)

Я буду использовать следующее изображение, чтобы показать вам, как использовать ControlNet. Вы можете скачать изображение, используя кнопку загрузки, чтобы следовать инструкциям.
...
**ControlNet нужно будет использовать с моделью Stable Diffusion**. В выпадающем меню **Stable Diffusion checkpoint** выберите модель, которую вы хотите использовать с ControlNet. Выберите v1-5-pruned-emaonly.ckpt, чтобы использовать [базовую модель v1.5](https://stable-diffusion-art.com/models/#Stable_diffusion_v15).

![](https://stable-diffusion-art.com/wp-content/uploads/2023/02/image-140.png)

На вкладке **txt2image** напишите **промпт** и, при желании, негативный промпт, которые будут использоваться ControlNet. Я буду использовать промпты ниже.
...
Вы должны увидеть, что сгенерированные изображения следуют позе входного изображения. Последнее изображение - это результат этапа предобработки. В данном случае это обнаруженные ключевые точки.
...
* Какие препроцессоры доступны (их много!)
* Настройки ControlNet

## Препроцессоры и модели

Первый шаг использования ControlNet - выбрать препроцессор. Полезно **включить предварительный просмотр**, чтобы вы знали, что делает препроцессор. После завершения предобработки исходное изображение отбрасывается, и только предварительно обработанное изображение будет использоваться для ControlNet.

Чтобы включить предварительный просмотр:

 1. Выберите **Allow Preview**.
 2. При желании выберите **Pixel Perfect**. ControlNet будет использовать заданную вами высоту и ширину изображения в text-to-image для генерации предварительно обработанного изображения.
...
Некоторые модели Control могут слишком сильно влиять на изображение. Уменьшите **Control Weight**, если вы видите проблемы с цветом или другие артефакты.

### Выбор правильной модели

После выбора препроцессора вы должны выбрать правильную модель.

В v1.1 легко определить, какая модель подходит. Все, что вам нужно сделать, это выбрать модель с тем же начальным ключевым словом, что и у препроцессора.
...
Есть несколько препроцессоров OpenPose.
...
Совет: Используйте Dw OpenPose для извлечения всех деталей.
...
См. примеры из text-to-image ниже. Положение тела не задаётся.
...
Препроцессоры Reference НЕ используют отдельную модель. Вам нужно только выбрать препроцессор, но не модель. (После выбора препроцессора reference выпадающее меню модели будет скрыто.)
...
Выберите один из этих препроцессоров для использования.
...
В разделе ControlNet загрузите изображение на холст.
...
[Детектор краев Canny](https://en.wikipedia.org/wiki/Canny_edge_detector) - это универсальный детектор краев ещё старой школы. Он извлекает контуры изображения. Он полезен для сохранения композиции исходного изображения.
...
* **Line art coarse**: Реалистичные линии с более толстыми линиями.
...
* **Normal Bae**: Оценка карты нормалей с использованием [метода неопределенности нормалей](https://github.com/baegwangbin/surface_normal_uncertainty), предложенного Bae и др.

#### Normal Midas

Подобно карте глубины Midas, карта нормалей Midas хороша для изоляции объекта от фона.
...
Карта нормалей Bae, как правило, отображает детали как на фоне, так и на переднем плане.

![](https://stable-diffusion-art.com/wp-content/uploads/2023/05/image-129.png)
![](https://stable-diffusion-art.com/wp-content/uploads/2023/05/image-130.png)
...
* **Scribble Pidinet**:[ Сеть Pixel Difference](https://github.com/zhuoinoulu/pidinet) (Pidinet) обнаруживает кривые и прямые края. Ее результат похож на HED, но обычно дает более чистые линии с меньшим количеством деталей.
* **Scribble xdog**: [E**X**tended **D**ifference **o**f **G**aussian](https://users.cs.northwestern.edu/~sco590/winnemoeller-cag2012.pdf) (XDoG) - это метод обнаружения краев. Важно настроить **порог xDoG** и наблюдать за результатом работы препроцессора.
...
Uniformer точно помечает все в этом примере.
...
Сегментация - мощная техника. Вы можете дополнительно манипулировать картой сегментации, чтобы размещать объекты в конкретрых местах. Используйте [цветовую карту](https://docs.google.com/spreadsheets/d/1se8YEtb2detS7OuPE86fXGyD269pMycAWe2mtKUj2W8/edit#gid=0) для ADE20k.
...
Изображение ниже получено только с моделью ControlNet Shuffle (Препроцессор: None). Композиция изображения ближе к оригиналу. Цветовая схема похожа на перемешанную.
...
Вы также можете использовать препроцессор None для этой модели T2I color.
...
![](https://stable-diffusion-art.com/wp-content/uploads/2023/05/image-153.png)

![](https://stable-diffusion-art.com/wp-content/uploads/2023/05/image-154.png)
...
ControlNet inpainting позволяет использовать высокий денойз (denoising strength) при [инпейнтинге](https://stable-diffusion-art.com/inpainting_basics/) для генерации множества вариаций без ущерба для согласованности с картиной в целом.
...
У меня есть вот эта картинка, и я хочу поменять лицо при помощи [инпеинта](https://stable-diffusion-art.com/inpainting_basics/).
...
ControlNet Inpainting - то, что вам нужно.
...
2. Используйте кисть, чтобы создать маску над областью, которую вы хотите перегенерировать. Если вы не знакомы с инпейнтингом, см. [руководство для начинающих по инпейнтингу](https://stable-diffusion-art.com/inpainting_basics/).
  ![](https://stable-diffusion-art.com/wp-content/uploads/2023/05/image-170.png)

3. Установите **Inpaint area** в **Only masked**. (**Whole picture** тоже будет работать)
...
В настоящее время есть 3 препроцессора инпейнтинга:

* **Inpaint_global_harmonious**: Улучшает глобальную согласованность и позволяет использовать высокую силу шумоподавления.
...
2. Загрузите [ip-adapter-plus-face_sd15.bin](https://huggingface.co/h94/IP-Adapter/blob/main/models/ip-adapter-plus-face_sd15.bin) и поместите его в **stable-diffusion-webui > models > ControlNet**.
...
Когда впервые используешь ControlNet, можно немного офигеть от количества настроек! Но давай-ка пройдемся по ним не спеша, разберем на пальцах.

Будет глубокое погружение в тему. Если надо в туалет, сходи сейчас...
...
Я использую вот эту картинку, чтобы показать влияние параметра **Control Weight**. На ней девушка сидит.
...
**Weight**: Насколько сильно карта контроля влияет на результат по сравнению с текстовым промптом. Похоже на [вес ключевых слов](https://stable-diffusion-art.com/prompt-guide/#Keyword_weight) в промпте, но применяется к карте контроля.
...
Как видно, **Control Weight определяет, насколько сильно карта контроля влияет на результат относительно промпта**. Чем меньше вес, тем меньше ControlNet требует от картинки следовать карте контроля.
...
**Balanced**: ControlNet применяется и при [conditioning](https://stable-diffusion-art.com/how-stable-diffusion-work/#Conditioning), и при [unconditioning](https://stable-diffusion-art.com/how-negative-prompt-work/#Sampling_with_negative_prompt) на [шаге семплирования](https://stable-diffusion-art.com/samplers/). Это стандартный режим работы.
...
**ControlNet is more important**: Выключает ControlNet на unconditioning. По факту параметр [CFG scale](https://stable-diffusion-art.com/how-stable-diffusion-work/#What_is_CFG_value) так же работает как множитель для эффекта ControlNet.
...
Resize mode определяет, что делать, когда размер входного изображения или карты контроля отличается от размера генерируемых картинок. Можно не заморачиваться с этими настройками, если соотношение сторон одинаковое.

Продемонстрирую эффект режимов ресайза, выставив генерацию горизонтальной картинки при вертикальном входном изображении/карте контроля.

**Just Resize**: Независимо масштабирует ширину и высоту карты контроля, чтобы вписать ее в холст. Меняет соотношение сторон карты.
